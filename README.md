...
-   main function: Estim() estimates the parameters by various methods.
    Also gives the associated asymptotic properties of the estimators.
-   estimation functions for specific methods: these functions are
    called by Estim() but can be used directly, as well. The methods
    provided so far are:
    -   the maximum-likelihood (MLParametersEstim()),
    -   the generalised method of moments with a finite
        (GMMParametersEstim()) or continuum moment conditions
        (CgmmParametersEstim()),
...
You can install the development version of tillsfunctionpackage from
