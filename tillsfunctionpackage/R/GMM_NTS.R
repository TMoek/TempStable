##### main function#####
GMMParametersEstim_NTS <- function(x, algo = c("2SGMM", "ITGMM", "CueGMM"), alphaReg = 0.01, regularization = c("Tikhonov",
    ...
    t_init <- getTime_()
    method <- getGMMmethodName_NTS(algo = algo, alphaReg = alphaReg, regularization = regularization, WeightingMatrix = WeightingMatrix,
        t_scheme = t_scheme, ...)
    Estim <- switch(algo, `2SGMM` = {
        Compute2SGMMParametersEstim_NTS(x = x, theta0 = theta0, alphaReg = alphaReg, regularization = regularization, WeightingMatrix = WeightingMatrix,
            t_scheme = t_scheme, eps = eps, ...)
    }, ITGMM = {
        ComputeITGMMParametersEstim_NTS(x = x, theta0 = theta0, alphaReg = alphaReg, regularization = regularization, WeightingMatrix = WeightingMatrix,
            t_scheme = t_scheme, IterationControl = IterationControl, eps = eps, ...)
    }, CueGMM = {
        ComputeCueGMMParametersEstim_NTS(x = x, theta0 = theta0, alphaReg = alphaReg, regularization = regularization,
    ...
    if (PrintTime) {
        CallingFct <- paste("Normal", "GMMParametersEstim", algo, t_scheme, sep = "_")
...
##### auxiliaries#####
getGMMmethodName_NTS <- function(algo, alphaReg, regularization, WeightingMatrix, t_scheme, ...) {
    args <- list(...)
...
Compute2SGMMParametersEstim_NTS <- function(x, theta0, alphaReg, regularization, WeightingMatrix, t_scheme, eps, ...) {
    iter = 0
    AllCurrentEstim <- ComputeCurrentEstim_NTS(t_scheme = t_scheme, theta0 = theta0, x = x, alphaReg = alphaReg, regularization = regularization,
        WeightingMatrix = "Id", eps = eps, ...)
    theta1 <- (AllCurrentEstim$OptInfo)$par
    t <- AllCurrentEstim$t
    ProvidedWeightingMatrix <- ComputeWeightingMatrix_NTS(t, theta1, x, WeightingMatrix, ...)
    CurrentEstimOptInfo <- ComputeCurrentEstim_NTS(t_scheme = t_scheme, theta0 = theta1, x = x, alphaReg = alphaReg, regularization = regularization,
...
ComputeITGMMParametersEstim_NTS <- function(x, theta0, alphaReg, regularization, WeightingMatrix, t_scheme, IterationControl,
    eps, ...) {
    iter = 0
    ...
    while ((iter < Control$NbIter) && (RelativeErr > Control$RelativeErrMax)) {
        ProvidedWeightingMatrix <- ComputeWeightingMatrix_NTS(t = t, theta = PrevEstimParVal, x = x, WeightingMatrix = WeightingMatrix,
            ...)
        AllCurrentEstim <- ComputeCurrentEstim_NTS(t_scheme = t_scheme, theta0 = PrevEstimParVal, x = x, alphaReg = alphaReg,
...
ComputeCueGMMParametersEstim_NTS <- function(x, theta0, alphaReg, regularization, WeightingMatrix, t_scheme, IterationControl,
    eps, ...) {
    iter = 0
    Control <- checkIterationControl(IterationControl)
...
##### current step####
ComputeCurrentEstim_NTS <- function(t_scheme, theta0, x, alphaReg, regularization, WeightingMatrix, eps, ...) {
    t <- ComputeT_NTS(tScheme = t_scheme, theta = theta0, x = x, alphaReg = alphaReg, regularization = regularization,
        WeightingMatrix = WeightingMatrix, eps = eps, ...)
    optOutput <- nlminb(start = theta0, objective = ComputeObjective_NTS, gradient = NULL, hessian = NULL, t = t, x = x,
...
ComputeObjective_NTS <- function(theta, t, x, alphaReg, regularization, WeightingMatrix, t_scheme, eps, ...) {
    K <- ComputeWeightingMatrix_NTS(t = t, theta = theta, x = x, WeightingMatrix = WeightingMatrix, ...)
    gbar <- sampleRealCFMoment_NTS(x = x, t = t, theta = theta)
    K1gbar <- ComputeInvKbyG_NTS(K = K, G = gbar, alphaReg = alphaReg, regularization = regularization, eps = eps)
...
ComputeWeightingMatrix_NTS <- function(t, theta, x, WeightingMatrix, ...) {
...
DataVarRealCFMoment_NTS <- function(t, theta, x) {
    gt <- DataMatrixRealCFMomentCondition_NTS(t = t, theta = theta, x = x)
    var(gt)
}
...
##### compute Inverse K#####
ComputeInvKbyG_NTS <- function(K, G, alphaReg, regularization, eps) {
    ...
    if (ComputeRegularized)
        K1G <- ComputeRegularizedK1G_NTS(K = K, G = G, alphaReg = alphaReg, regularization = regularization, eps = eps)
    K1G
}
...
ComputeRegularizedK1G_NTS <- function(K, G, alphaReg, regularization, eps) {
    RegularisedSol_NTS(Kn = K, alphaReg = alphaReg, r = G, regularization = regularization, eps = eps)
}

RegularisedSol_NTS <- function(Kn, alphaReg, r, regularization = c("Tikhonov", "LF", "cut-off"), eps, ...) {
    regularization <- match.arg(regularization)
    return(ComputeRegularizedSol_NTS(Kn = Kn, alpha = alphaReg, r = r, regularization = regularization, eps = eps, ...))
}

ComputeRegularizedSol_NTS <- function(Kn, alpha, r, regularization = c("Tikhonov", "LF", "cut-off"), eps, ...) {
    regularization <- match.arg(regularization)
    ...
    qAlphaLambda <- ComputeqAlphaLambda_NTS(lambda = lambda, alpha = alpha, regularization = regularization, eps = eps,
        ..., Kn = Kn)
...
ComputeqAlphaLambda_NTS <- function(lambda, alpha, regularization, eps, ...) {
...
ComputeT_NTS <- function(tScheme, eps, ...) {
    ...
    if (tScheme == "free") {
        ...
        checkMainArgs(args)
        t <- ComputeEquallySpacedPoints_NTS(tScheme = tScheme, eps = eps, ...)
    } else if ((tScheme == "uniformOpt") || (tScheme == "ArithOpt") || (tScheme == "VarOpt")) {
        nb_t <- checkMainArgs(args)
        t <- ComputeOptimisedPoints_NTS(tScheme = tScheme, eps = eps, ...)
    }
    t
}
...
ComputeEquallySpacedPoints_NTS <- function(..., tScheme, eps, x, nb_t, Constrained = TRUE) {
    Bands <- Compute_tBands_NTS(x = x, Constrained = Constrained, eps = eps, ...)
    if (tScheme == "equally") {
...
ComputeOptimisedPoints_NTS <- function(..., tScheme, eps, nb_t = 40, Constrained = TRUE, FastOptim = TRUE) {
    An <- ComputeFirstRootRealeCF(...)
    t0 <- seq(eps, An - eps, length.out = nb_t)
    if ((tScheme == "uniformOpt") || (tScheme == "ArithOpt")) {
        t <- ComputeApproxOptSpacing_NTS(..., tScheme = tScheme, eps = eps, nb_t = nb_t, Constrained = Constrained, An = An)
    } else if (tScheme == "VarOpt") {
        # not done yet!
        if (FastOptim)
            t <- optim(par = t0, fn = ObjectiveFctToMinIn_t_NTS, gr = NULL, ..., method = "Nelder-Mead")$par else t <- nlminb(start = t0, objective = ObjectiveFctToMinIn_t_NTS, ...)
    }
    t
}

ComputeApproxOptSpacing_NTS <- function(..., tScheme, eps, nb_t, Constrained, An) {
    tau0 <- ComputeTau0(An = An, tScheme = tScheme, eps = eps, nb_t = nb_t)
    if (Constrained) {
        Bands <- Compute_tauBands_NTS(tScheme, eps, nb_t, An)
        tauInfo <- nlminb(start = tau0, objective = ObjectiveFctToMinIn_tau_NTS, gradient = NULL, hessian = NULL, tScheme = tScheme,
...
ObjectiveFctToMinIn_tau_NTS <- function(tau, ..., tScheme, nb_t, theta, x, WeightingMatrix, eps, alphaReg = 0.01, regularization = "Tikhonov",
    fctToApply = InvDet) {
    if (tScheme == "uniformOpt") {
        t <- (1:nb_t) * tau
    } else if (tScheme == "ArithOpt") {
    ...
    ObjectiveFctToMinIn_t_NTS(t, ..., theta = theta, x = x, WeightingMatrix = WeightingMatrix, eps = eps, alphaReg = alphaReg,
        regularization = regularization, fctToApply = fctToApply)
}


ObjectiveFctToMinIn_t_NTS <- function(t, ..., theta, x, WeightingMatrix, eps, alphaReg = 0.01, regularization = "Tikhonov",
    fctToApply = InvDet) {
    W <- GMMasymptoticVarianceEstim_NTS(..., t = t, theta = theta, x = x, WeightingMatrix = WeightingMatrix, eps = eps,
        alphaReg = alphaReg, regularization = regularization)
    fctToApply(W)
}


GMMasymptoticVarianceEstim_NTS <- function(..., t, theta, x, WeightingMatrix, eps, alphaReg = 0.01, regularization = "Tikhonov") {
    K <- ComputeWeightingMatrix_NTS(t = t, theta = theta, x = x, WeightingMatrix = WeightingMatrix, ...)
    B <- jacobianSampleRealCFMoment_NTS(t, theta)
    fct <- function(G) ComputeInvKbyG_NTS(K = K, G = G, alphaReg = alphaReg, regularization = regularization, eps = eps)
...
jacobianComplexCF_NTS <- function(t, theta) {
    ComplexCFtoDeriv <- function(th) ComplexCF_NTS(t, th)
