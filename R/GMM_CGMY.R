##### main function#####
GMMParametersEstim_CGMY <- function(x, algo = c("2SGMM", "ITGMM", "CueGMM"), alphaReg = 0.01, regularization = c("Tikhonov",
    ...
    t_init <- getTime_()
    method <- getGMMmethodName_CGMY(algo = algo, alphaReg = alphaReg, regularization = regularization, WeightingMatrix = WeightingMatrix,
        t_scheme = t_scheme, ...)
    Estim <- switch(algo, `2SGMM` = {
        Compute2SGMMParametersEstim_CGMY(x = x, theta0 = theta0, alphaReg = alphaReg, regularization = regularization,
            WeightingMatrix = WeightingMatrix, t_scheme = t_scheme, eps = eps, ...)
    }, ITGMM = {
        ComputeITGMMParametersEstim_CGMY(x = x, theta0 = theta0, alphaReg = alphaReg, regularization = regularization,
            WeightingMatrix = WeightingMatrix, t_scheme = t_scheme, IterationControl = IterationControl, eps = eps, ...)
    }, CueGMM = {
        ComputeCueGMMParametersEstim_CGMY(x = x, theta0 = theta0, alphaReg = alphaReg, regularization = regularization,
            WeightingMatrix = WeightingMatrix, t_scheme = t_scheme, IterationControl = IterationControl, eps = eps, ...)
    }, stop(paste(algo, " not taken into account !")))
    if (PrintTime) {
        CallingFct <- paste("CGMY", "GMMParametersEstim", algo, t_scheme, sep = "_")
...
##### auxiliaries#####
getGMMmethodName_CGMY <- function(algo, alphaReg, regularization, WeightingMatrix, t_scheme, ...) {
    args <- list(...)
...
Compute2SGMMParametersEstim_CGMY <- function(x, theta0, alphaReg, regularization, WeightingMatrix, t_scheme, eps, ...) {
    iter = 0
    AllCurrentEstim <- ComputeCurrentEstim_CGMY(t_scheme = t_scheme, theta0 = theta0, x = x, alphaReg = alphaReg, regularization = regularization,
        WeightingMatrix = "Id", eps = eps, ...)
    theta1 <- (AllCurrentEstim$OptInfo)$par
    t <- AllCurrentEstim$t
    ProvidedWeightingMatrix <- ComputeWeightingMatrix_CGMY(t, theta1, x, WeightingMatrix, ...)
    CurrentEstimOptInfo <- ComputeCurrentEstim_CGMY(t_scheme = t_scheme, theta0 = theta1, x = x, alphaReg = alphaReg, regularization = regularization,
...
ComputeITGMMParametersEstim_CGMY <- function(x, theta0, alphaReg, regularization, WeightingMatrix, t_scheme, IterationControl,
    eps, ...) {
    iter = 0
    ...
    while ((iter < Control$NbIter) && (RelativeErr > Control$RelativeErrMax)) {
        ProvidedWeightingMatrix <- ComputeWeightingMatrix_CGMY(t = t, theta = PrevEstimParVal, x = x, WeightingMatrix = WeightingMatrix,
            ...)
        AllCurrentEstim <- ComputeCurrentEstim_CGMY(t_scheme = t_scheme, theta0 = PrevEstimParVal, x = x, alphaReg = alphaReg,
...
ComputeCueGMMParametersEstim_CGMY <- function(x, theta0, alphaReg, regularization, WeightingMatrix, t_scheme, IterationControl,
    eps, ...) {
    iter = 0
    Control <- checkIterationControl(IterationControl)
...
##### current step####
ComputeCurrentEstim_CGMY <- function(t_scheme, theta0, x, alphaReg, regularization, WeightingMatrix, eps, ...) {
    t <- ComputeT_CGMY(tScheme = t_scheme, theta = theta0, x = x, alphaReg = alphaReg, regularization = regularization,
        WeightingMatrix = WeightingMatrix, eps = eps, ...)
    optOutput <- nlminb(start = theta0, objective = ComputeObjective_CGMY, gradient = NULL, hessian = NULL, t = t, x = x,
...
ComputeObjective_CGMY <- function(theta, t, x, alphaReg, regularization, WeightingMatrix, t_scheme, eps, ...) {
    K <- ComputeWeightingMatrix_CGMY(t = t, theta = theta, x = x, WeightingMatrix = WeightingMatrix, ...)
    gbar <- sampleRealCFMoment_CGMY(x = x, t = t, theta = theta)
    K1gbar <- ComputeInvKbyG_CGMY(K = K, G = gbar, alphaReg = alphaReg, regularization = regularization, eps = eps)
...
ComputeWeightingMatrix_CGMY <- function(t, theta, x, WeightingMatrix, ...) {
...
DataVarRealCFMoment_CGMY <- function(t, theta, x) {
    gt <- DataMatrixRealCFMomentCondition_CGMY(t = t, theta = theta, x = x)
    var(gt)
}
...
##### compute Inverse K#####
ComputeInvKbyG_CGMY <- function(K, G, alphaReg, regularization, eps) {
    ...
    if (ComputeRegularized)
        K1G <- ComputeRegularizedK1G_CGMY(K = K, G = G, alphaReg = alphaReg, regularization = regularization, eps = eps)
    K1G
}
...
ComputeRegularizedK1G_CGMY <- function(K, G, alphaReg, regularization, eps) {
    RegularisedSol_CGMY(Kn = K, alphaReg = alphaReg, r = G, regularization = regularization, eps = eps)
}

RegularisedSol_CGMY <- function(Kn, alphaReg, r, regularization = c("Tikhonov", "LF", "cut-off"), eps, ...) {
    regularization <- match.arg(regularization)
    return(ComputeRegularizedSol_CGMY(Kn = Kn, alpha = alphaReg, r = r, regularization = regularization, eps = eps, ...))
}

ComputeRegularizedSol_CGMY <- function(Kn, alpha, r, regularization = c("Tikhonov", "LF", "cut-off"), eps, ...) {
    regularization <- match.arg(regularization)
    ...
    qAlphaLambda <- ComputeqAlphaLambda_CGMY(lambda = lambda, alpha = alpha, regularization = regularization, eps = eps,
        ..., Kn = Kn)
...
ComputeqAlphaLambda_CGMY <- function(lambda, alpha, regularization, eps, ...) {
...
ComputeT_CGMY <- function(tScheme, eps, ...) {
    ...
    if (tScheme == "free") {
        ...
        checkMainArgs(args)
        t <- ComputeEquallySpacedPoints_CGMY(tScheme = tScheme, eps = eps, ...)
    } else if ((tScheme == "uniformOpt") || (tScheme == "ArithOpt") || (tScheme == "VarOpt")) {
        nb_t <- checkMainArgs(args)
        t <- ComputeOptimisedPoints_CGMY(tScheme = tScheme, eps = eps, ...)
    }
    t
}
...
ComputeEquallySpacedPoints_CGMY <- function(..., tScheme, eps, x, nb_t, Constrained = TRUE) {
    Bands <- Compute_tBands_CGMY(x = x, Constrained = Constrained, eps = eps, ...)
    if (tScheme == "equally") {
...
ComputeOptimisedPoints_CGMY <- function(..., tScheme, eps, nb_t = 40, Constrained = TRUE, FastOptim = TRUE) {
    An <- ComputeFirstRootRealeCF(...)
    t0 <- seq(eps, An - eps, length.out = nb_t)
    if ((tScheme == "uniformOpt") || (tScheme == "ArithOpt")) {
        t <- ComputeApproxOptSpacing_CGMY(..., tScheme = tScheme, eps = eps, nb_t = nb_t, Constrained = Constrained, An = An)
    } else if (tScheme == "VarOpt") {
        # not done yet!
        if (FastOptim)
            t <- optim(par = t0, fn = ObjectiveFctToMinIn_t_CGMY, gr = NULL, ..., method = "Nelder-Mead")$par else t <- nlminb(start = t0, objective = ObjectiveFctToMinIn_t_CGMY, ...)
    }
    t
}

ComputeApproxOptSpacing_CGMY <- function(..., tScheme, eps, nb_t, Constrained, An) {
    tau0 <- ComputeTau0(An = An, tScheme = tScheme, eps = eps, nb_t = nb_t)
    if (Constrained) {
        Bands <- Compute_tauBands_CGMY(tScheme, eps, nb_t, An)
        tauInfo <- nlminb(start = tau0, objective = ObjectiveFctToMinIn_tau_CGMY, gradient = NULL, hessian = NULL, tScheme = tScheme,
...
ObjectiveFctToMinIn_tau_CGMY <- function(tau, ..., tScheme, nb_t, theta, x, WeightingMatrix, eps, alphaReg = 0.01, regularization = "Tikhonov",
    fctToApply = InvDet) {
    if (tScheme == "uniformOpt") {
        t <- (1:nb_t) * tau
    } else if (tScheme == "ArithOpt") {
    ...
    ObjectiveFctToMinIn_t_CGMY(t, ..., theta = theta, x = x, WeightingMatrix = WeightingMatrix, eps = eps, alphaReg = alphaReg,
        regularization = regularization, fctToApply = fctToApply)
}


ObjectiveFctToMinIn_t_CGMY <- function(t, ..., theta, x, WeightingMatrix, eps, alphaReg = 0.01, regularization = "Tikhonov",
    fctToApply = InvDet) {
    W <- GMMasymptoticVarianceEstim_CGMY(..., t = t, theta = theta, x = x, WeightingMatrix = WeightingMatrix, eps = eps,
        alphaReg = alphaReg, regularization = regularization)
    fctToApply(W)
}


GMMasymptoticVarianceEstim_CGMY <- function(..., t, theta, x, WeightingMatrix, eps, alphaReg = 0.01, regularization = "Tikhonov") {
    K <- ComputeWeightingMatrix_CGMY(t = t, theta = theta, x = x, WeightingMatrix = WeightingMatrix, ...)
    B <- jacobianSampleRealCFMoment_CGMY(t, theta)
    fct <- function(G) ComputeInvKbyG_CGMY(K = K, G = G, alphaReg = alphaReg, regularization = regularization, eps = eps)
...
jacobianComplexCF_CGMY <- function(t, theta) {
    ComplexCFtoDeriv <- function(th) ComplexCF_CGMY(t, th)
