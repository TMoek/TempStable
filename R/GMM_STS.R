##### main function#####
GMMParametersEstim_STS <- function(x, algo = c("2SGMM", "ITGMM", "CueGMM"), alphaReg = 0.01, regularization = c("Tikhonov",
    ...
    t_init <- getTime_()
    method <- getGMMmethodName_STS(algo = algo, alphaReg = alphaReg, regularization = regularization, WeightingMatrix = WeightingMatrix,
        t_scheme = t_scheme, ...)
    Estim <- switch(algo, `2SGMM` = {
        Compute2SGMMParametersEstim_STS(x = x, theta0 = theta0, alphaReg = alphaReg, regularization = regularization, WeightingMatrix = WeightingMatrix,
            t_scheme = t_scheme, eps = eps, ...)
    }, ITGMM = {
        ComputeITGMMParametersEstim_STS(x = x, theta0 = theta0, alphaReg = alphaReg, regularization = regularization, WeightingMatrix = WeightingMatrix,
            t_scheme = t_scheme, IterationControl = IterationControl, eps = eps, ...)
    }, CueGMM = {
        ComputeCueGMMParametersEstim_STS(x = x, theta0 = theta0, alphaReg = alphaReg, regularization = regularization,
    ...
    if (PrintTime) {
        CallingFct <- paste("Subordinator", "GMMParametersEstim", algo, t_scheme, sep = "_")
...
##### auxiliaries#####
getGMMmethodName_STS <- function(algo, alphaReg, regularization, WeightingMatrix, t_scheme, ...) {
    args <- list(...)
...
checkIterationControl <- function(IterationControl) {
    NbIter <- ifelse(is.null(IterationControl$NbIter), 10, IterationControl$NbIter)
    PrintIter <- ifelse(is.null(IterationControl$PrintIter), TRUE, IterationControl$PrintIter)
    RelativeErrMax <- ifelse(is.null(IterationControl$RelativeErrMax), 0.001, IterationControl$RelativeErrMax)
    list(NbIter = NbIter, PrintIter = PrintIter, RelativeErrMax = RelativeErrMax)
}

PrintIteration <- function(theta, iter, nbIterMax) {
    print(paste("---------------iteration ", iter, "/", nbIterMax, "------------------------ (", theta[1], ",", theta[2],
        ",", theta[3], ")"))
    cat(" \n")
}
...
Compute2SGMMParametersEstim_STS <- function(x, theta0, alphaReg, regularization, WeightingMatrix, t_scheme, eps, ...) {
    iter = 0
    AllCurrentEstim <- ComputeCurrentEstim_STS(t_scheme = t_scheme, theta0 = theta0, x = x, alphaReg = alphaReg, regularization = regularization,
        WeightingMatrix = "Id", eps = eps, ...)
    theta1 <- (AllCurrentEstim$OptInfo)$par
    t <- AllCurrentEstim$t
    ProvidedWeightingMatrix <- ComputeWeightingMatrix_STS(t, theta1, x, WeightingMatrix, ...)
    CurrentEstimOptInfo <- ComputeCurrentEstim_STS(t_scheme = t_scheme, theta0 = theta1, x = x, alphaReg = alphaReg, regularization = regularization,
...
ComputeITGMMParametersEstim_STS <- function(x, theta0, alphaReg, regularization, WeightingMatrix, t_scheme, IterationControl,
    eps, ...) {
    iter = 0
    ...
    while ((iter < Control$NbIter) && (RelativeErr > Control$RelativeErrMax)) {
        ProvidedWeightingMatrix <- ComputeWeightingMatrix_STS(t = t, theta = PrevEstimParVal, x = x, WeightingMatrix = WeightingMatrix,
            ...)
        AllCurrentEstim <- ComputeCurrentEstim_STS(t_scheme = t_scheme, theta0 = PrevEstimParVal, x = x, alphaReg = alphaReg,
...
ComputeCueGMMParametersEstim_STS <- function(x, theta0, alphaReg, regularization, WeightingMatrix, t_scheme, IterationControl,
    eps, ...) {
    iter = 0
    Control <- checkIterationControl(IterationControl)
...
##### current step####
ComputeCurrentEstim_STS <- function(t_scheme, theta0, x, alphaReg, regularization, WeightingMatrix, eps, ...) {
    t <- ComputeT_STS(tScheme = t_scheme, theta = theta0, x = x, alphaReg = alphaReg, regularization = regularization,
        WeightingMatrix = WeightingMatrix, eps = eps, ...)
    optOutput <- nlminb(start = theta0, objective = ComputeObjective_STS, gradient = NULL, hessian = NULL, t = t, x = x,
...
ComputeObjective_STS <- function(theta, t, x, alphaReg, regularization, WeightingMatrix, t_scheme, eps, ...) {
    K <- ComputeWeightingMatrix_STS(t = t, theta = theta, x = x, WeightingMatrix = WeightingMatrix, ...)
    gbar <- sampleRealCFMoment_STS(x = x, t = t, theta = theta)
    K1gbar <- ComputeInvKbyG_STS(K = K, G = gbar, alphaReg = alphaReg, regularization = regularization, eps = eps)
...
ComputeWeightingMatrix_STS <- function(t, theta, x, WeightingMatrix, ...) {
...
DataVarRealCFMoment_STS <- function(t, theta, x) {
    gt <- DataMatrixRealCFMomentCondition_STS(t = t, theta = theta, x = x)
    var(gt)
}
...
##### compute Inverse K#####
ComputeInvKbyG_STS <- function(K, G, alphaReg, regularization, eps) {
    ...
    if (ComputeRegularized)
        K1G <- ComputeRegularizedK1G_STS(K = K, G = G, alphaReg = alphaReg, regularization = regularization, eps = eps)
    K1G
}
...
ComputeRegularizedK1G_STS <- function(K, G, alphaReg, regularization, eps) {
    RegularisedSol_STS(Kn = K, alphaReg = alphaReg, r = G, regularization = regularization, eps = eps)
}

RegularisedSol_STS <- function(Kn, alphaReg, r, regularization = c("Tikhonov", "LF", "cut-off"), eps, ...) {
    regularization <- match.arg(regularization)
    return(ComputeRegularizedSol_STS(Kn = Kn, alpha = alphaReg, r = r, regularization = regularization, eps = eps, ...))
}

ComputeRegularizedSol_STS <- function(Kn, alpha, r, regularization = c("Tikhonov", "LF", "cut-off"), eps, ...) {
    regularization <- match.arg(regularization)
    ...
    qAlphaLambda <- ComputeqAlphaLambda_STS(lambda = lambda, alpha = alpha, regularization = regularization, eps = eps,
        ..., Kn = Kn)
...
ComputeqAlphaLambda_STS <- function(lambda, alpha, regularization, eps, ...) {
...
ComputeT_STS <- function(tScheme, eps, ...) {
    ...
    if (tScheme == "free") {
        ...
        checkMainArgs(args)
        t <- ComputeEquallySpacedPoints_STS(tScheme = tScheme, eps = eps, ...)
    } else if ((tScheme == "uniformOpt") || (tScheme == "ArithOpt") || (tScheme == "VarOpt")) {
        nb_t <- checkMainArgs(args)
        t <- ComputeOptimisedPoints_STS(tScheme = tScheme, eps = eps, ...)
    }
    t
}
...
ComputeEquallySpacedPoints_STS <- function(..., tScheme, eps, x, nb_t, Constrained = TRUE) {
    Bands <- Compute_tBands_STS(x = x, Constrained = Constrained, eps = eps, ...)
    if (tScheme == "equally") {
...
ComputeOptimisedPoints_STS <- function(..., tScheme, eps, nb_t = 40, Constrained = TRUE, FastOptim = TRUE) {
    An <- ComputeFirstRootRealeCF(...)
    t0 <- seq(eps, An - eps, length.out = nb_t)
    if ((tScheme == "uniformOpt") || (tScheme == "ArithOpt")) {
        t <- ComputeApproxOptSpacing_STS(..., tScheme = tScheme, eps = eps, nb_t = nb_t, Constrained = Constrained, An = An)
    } else if (tScheme == "VarOpt") {
        # not done yet!
        if (FastOptim)
            t <- optim(par = t0, fn = ObjectiveFctToMinIn_t_STS, gr = NULL, ..., method = "Nelder-Mead")$par else t <- nlminb(start = t0, objective = ObjectiveFctToMinIn_t_STS, ...)
    }
    t
}

ComputeApproxOptSpacing_STS <- function(..., tScheme, eps, nb_t, Constrained, An) {
    tau0 <- ComputeTau0(An = An, tScheme = tScheme, eps = eps, nb_t = nb_t)
    if (Constrained) {
        Bands <- Compute_tauBands_STS(tScheme, eps, nb_t, An)
        tauInfo <- nlminb(start = tau0, objective = ObjectiveFctToMinIn_tau_STS, gradient = NULL, hessian = NULL, tScheme = tScheme,
...
ComputeTau0 <- function(An, tScheme, eps, nb_t) {
    if (tScheme == "uniformOpt")
        tau0 <- An/(2 * nb_t) else if (tScheme == "ArithOpt")
...
ObjectiveFctToMinIn_tau_STS <- function(tau, ..., tScheme, nb_t, theta, x, WeightingMatrix, eps, alphaReg = 0.01, regularization = "Tikhonov",
    fctToApply = InvDet) {
    if (tScheme == "uniformOpt") {
        t <- (1:nb_t) * tau
    } else if (tScheme == "ArithOpt") {
    ...
    ObjectiveFctToMinIn_t_STS(t, ..., theta = theta, x = x, WeightingMatrix = WeightingMatrix, eps = eps, alphaReg = alphaReg,
        regularization = regularization, fctToApply = fctToApply)
}

InvDet <- function(Mat) 1/abs(det(Mat))

ObjectiveFctToMinIn_t_STS <- function(t, ..., theta, x, WeightingMatrix, eps, alphaReg = 0.01, regularization = "Tikhonov",
    fctToApply = InvDet) {
    W <- GMMasymptoticVarianceEstim_STS(..., t = t, theta = theta, x = x, WeightingMatrix = WeightingMatrix, eps = eps,
        alphaReg = alphaReg, regularization = regularization)
    fctToApply(W)
}


GMMasymptoticVarianceEstim_STS <- function(..., t, theta, x, WeightingMatrix, eps, alphaReg = 0.01, regularization = "Tikhonov") {
    K <- ComputeWeightingMatrix_STS(t = t, theta = theta, x = x, WeightingMatrix = WeightingMatrix, ...)
...
jacobianComplexCF_STS <- function(t, theta) {
    ComplexCFtoDeriv <- function(th) ComplexCF_STS(t, th)
