##### main function#####
GMCParametersEstim_CGMY <- function(x, algo = c("2SGMC", "ITGMC", "CueGMC"), ncond, alphaReg = 0.01, regularization = c("Tikhonov",
    ...
    t_init <- getTime_()
    method <- getGMCmethodName_CGMY(algo = algo, ncond = ncondfl, alphaReg = alphaReg, regularization = regularization,
        WeightingMatrix = WeightingMatrix, ...)
    Estim <- switch(algo, `2SGMC` = {
        Compute2SGMCParametersEstim_CGMY(x = x, ncond = ncondfl, theta0 = theta0, alphaReg = alphaReg, regularization = regularization,
            WeightingMatrix = WeightingMatrix, eps = eps, ...)
    }, ITGMM = {
        ComputeITGMCParametersEstim_CGMY(x = x, ncond = ncondfl, theta0 = theta0, alphaReg = alphaReg, regularization = regularization,
            WeightingMatrix = WeightingMatrix, IterationControl = IterationControl, eps = eps, ...)
    }, CueGMM = {
        ComputeCueGMCParametersEstim_CGMY(x = x, ncond = ncondfl, theta0 = theta0, alphaReg = alphaReg, regularization = regularization,
            WeightingMatrix = WeightingMatrix, IterationControl = IterationControl, eps = eps, ...)
    }, stop(paste(algo, " not taken into account !")))
    if (PrintTime) {
        CallingFct <- paste("CGMY", "GMCParametersEstim", algo, "ncond=", ncondfl, sep = "_")
...
getGMCmethodName_CGMY <- function(algo, ncond, alphaReg, regularization, WeightingMatrix, ...) {
    args <- list(...)
...
##### GMC methods#####
Compute2SGMCParametersEstim_CGMY <- function(x, ncond, theta0, alphaReg, regularization, WeightingMatrix, eps, ...) {
    iter = 0
    AllCurrentEstim <- ComputeCurrentGMC_CGMY(theta0 = theta0, x = x, ncond = ncond, alphaReg = alphaReg, regularization = regularization,
        WeightingMatrix = "Id", eps = eps, ...)
    theta1 <- (AllCurrentEstim$OptInfo)$par
    ProvidedWeightingMatrix <- ComputeGMCWeightingMatrix_CGMY(theta1, x, ncond, WeightingMatrix, ...)
    CurrentEstimOptInfo <- ComputeCurrentGMC_CGMY(theta0 = theta1, x = x, ncond = ncond, alphaReg = alphaReg, regularization = regularization,
        WeightingMatrix = "provided", eps = eps, ..., ProvidedWeightingMatrix = ProvidedWeightingMatrix)$OptInfo
    list(Estim = CurrentEstimOptInfo)
}


ComputeITGMCParametersEstim_CGMY <- function(x, ncond, theta0, alphaReg, regularization, WeightingMatrix, IterationControl,
    eps, ...) {
    iter = 0
    ...
    while ((iter < Control$NbIter) && (RelativeErr > Control$RelativeErrMax)) {
        ProvidedWeightingMatrix <- ComputeWeightingMatrix_CGMY(theta = PrevEstimParVal, x = x, ncond = ncond, WeightingMatrix = WeightingMatrix,
            ...)
        AllCurrentEstim <- ComputeCurrentGMC_CGMY(theta0 = PrevEstimParVal, x = x, ncond = ncond, alphaReg = alphaReg,
...
ComputeCueGMCParametersEstim_CGMY <- function(x, ncond, theta0, alphaReg, regularization, WeightingMatrix, IterationControl,
    eps, ...) {
    iter = 0
    Control <- checkIterationControl(IterationControl)
...
##### current step####
ComputeCurrentGMC_CGMY <- function(theta0, x, ncond, alphaReg, regularization, WeightingMatrix, eps, ...) {
    optOutput <- nlminb(start = theta0, objective = ComputeGMCObjective_CGMY, gradient = NULL, hessian = NULL, x = x, ncond = ncond,
...
ComputeGMCObjective_CGMY <- function(theta, x, ncond, alphaReg, regularization, WeightingMatrix, eps, ...) {
    W <- ComputeGMCWeightingMatrix_CGMY(theta = theta, x = x, ncond = ncond, WeightingMatrix = WeightingMatrix, ...)
    gbar <- sampleCumulants_CGMY(x = x, ncond = ncond, theta = theta)
    W1gbar <- ComputeInvKbyG_CGMY(K = W, G = gbar, alphaReg = alphaReg, regularization = regularization, eps = eps)
...
jththeoreticalCumulant_CGMY <- function(j, theta) {
    gamma(j - theta[4]) * (theta[1]/theta[2]^(j - theta[4]) + (-1)^j * theta[1]/theta[3]^(j - theta[4]))
}
...
asymVarCumulants_CGMY <- function(theta, x, ncond) {
    ...
    gvec <- function(z) {
        sapply(X = (1:ncond), FUN = g, z = z) - theoreticalCumulants_CGMY(ncond = ncond, theta = theta)
    }
    gmat <- function(z) {
